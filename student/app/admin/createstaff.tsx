...
export default function CreateStaffScreen() {
  ...
  const handleCreate = async () => {
    const { username, email, password, first_name, last_name, department_id } = form;
    ...
    try {
      const token = await AsyncStorage.getItem("accessToken");
      await axios.post(`${API_BASE_URL}/api/accounts/create/dept-staff/`, form, {
        headers: { Authorization: `Bearer ${token}` }
      });
      Alert.alert("Success", "The new staff account has been created.");
      router.back();
    } catch (err: any) {
      Alert.alert("Error", "Could not create account. The username or email might already be in use.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={[styles.header, { backgroundColor: themeColors.headerBg, borderBottomColor: themeColors.border }]}>
          ...
          <View style={styles.headerTitleBox}>
            <Text style={[styles.headerTitle, { color: themeColors.text }]}>Add Staff</Text>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.identityShell, { borderBottomColor: themeColors.border }]}>
              <View style={[styles.suiteIcon, { backgroundColor: '#6366F115' }]}>
                <MaterialCommunityIcons name="account-plus" size={48} color="#6366F1" />
              </View>
              <Text style={[styles.suiteTitle, { color: themeColors.text }]}>Staff Details</Text>
              <Text style={[styles.suiteDesc, { color: themeColors.subText }]}>Fill in the form below to create a new staff account.</Text>
            </View>
            ...
            <View style={styles.formStack}>
              ...
              <View style={[styles.inputCluster, { marginTop: 35 }]}>
                ...
                <Text style={[styles.clusterLab, { color: themeColors.subText, marginTop: 20 }]}>SELECT DEPARTMENT</Text>
