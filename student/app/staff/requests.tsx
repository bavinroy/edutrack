...
export default function StaffRequestsScreen() {
  ...
  const renderRequest = ({ item }: { item: Request }) => {
    const pending = item.staff_status === "pending";

    return (
      <TouchableOpacity
        style={[
          styles.card,
          pending && { borderColor: "#30e4de", borderWidth: 2, backgroundColor: "rgba(48,228,222,0.1)" },
        ]}
        onPress={() => {
      ...
      >
        ...
        <Text style={styles.meta}>
          Student: {item.student_name} • Staff: {item.staff_status} • Admin: {item.admin_status}
        </Text>
        {item.staff_comment && <Text style={styles.comment}>Comment: {item.staff_comment}</Text>}
      </TouchableOpacity>
    );
  };
    ...
    <ImageBackground source={require("../../assets/images/back.jpg")} style={{ flex: 1 }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <ScrollView>
              ...
              <Text style={{ marginBottom: 10 }}>{currentRequest?.letter.content}</Text>
              <Text style={{ marginBottom: 5 }}>Student: {currentRequest?.student_name}</Text>
              ...
              <View style={{ flexDirection: "row", marginTop: 10 }}>
                ...
                <TouchableOpacity
                  style={[styles.button, { flex: 1, backgroundColor: "red", marginLeft: 5 }]}
                  onPress={() =>
                    currentRequest &&
      ...
      <View style={styles.bottomNav}>
        <Ionicons
          name="home"
          size={24}
          color={pathname === "/staff/dashboard" ? "#0e0e0dff" : "#fff"}
          onPress={() => router.push("/staff/dashboard")}
        />
        <Ionicons
          name="desktop-outline"
          size={24}
          color={pathname === "/staff/notice" ? "#0e0e0dff" : "#fff"}
          onPress={() => router.push("/staff/notice")}
        />
        <Ionicons
          name="person-circle-outline"
          size={24}
          color={pathname === "/staff/profile" ? "#0e0e0dff" : "#fff"}
...
const styles = StyleSheet.create({
  ...
  comment: { fontSize: 14, fontStyle: "italic", color: "#333", marginBottom: 6 },
  actions: { flexDirection: "row", marginTop: 8 },
  button: { flex: 1, marginHorizontal: 5, padding: 8, borderRadius: 6, justifyContent: "center" },
