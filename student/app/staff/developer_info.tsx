...
export default function StaffDeveloperInfoScreen() {
      ...
      <ScrollView contentContainerStyle={styles.scrollBody} showsVerticalScrollIndicator={false}>
        ...
        {developers.map((dev, idx) => (
          <View key={idx} style={[styles.devCard, { backgroundColor: themeColors.card, borderColor: themeColors.border }]}>
            <View style={[styles.devIconBox, { backgroundColor: isDark ? '#1E293B' : '#ECFDF5' }]}>
              <Ionicons name={dev.icon as any} size={24} color="#10B981" />
            </View>
            <View style={styles.devInfo}>
              <Text style={[styles.devName, { color: themeColors.text }]}>{dev.name}</Text>
              <Text style={[styles.devRole, { color: '#10B981' }]}>{dev.role}</Text>
        ...
        <TouchableOpacity
          style={[styles.contactCard, { backgroundColor: '#10B981' }]}
